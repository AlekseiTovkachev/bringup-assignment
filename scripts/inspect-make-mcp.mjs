...
const timeoutMs = Number(process.env.MAKE_MCP_INSPECT_TIMEOUT_MS || 45000);
...
const timeout = setTimeout(() => {
  child.kill("SIGTERM");
  fail(`Timed out after ${timeoutMs}ms while inspecting Make MCP.\n${stderr}`);
}, timeoutMs);
...
try {
  const init = await request("initialize", {
    ...
    capabilities: {},
    clientInfo: {
      name: "bringup-assignment-make-inspector",
      version: "1.0.0",
    },
  });
    ...
    JSON.stringify(
      {
        ...
        tools: (tools.tools || [])
          .filter((tool) => !nameFilter || tool.name.includes(nameFilter))
          .map((tool) => ({
            name: tool.name,
